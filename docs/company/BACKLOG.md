...
> Last updated: 2026-07-11 (run 20 — `/login` metadata: same App-Router `<Head>` no-op as the run-19 signup pages — `/login` is a Client Component whose `next/head` `<Head>` title/description were dead, so it served the generic root `<title>` with no canonical/OG despite being in `sitemap.ts` (priority 0.3). Added sibling server `app/login/layout.tsx` metadata carrier (title/desc/keywords/canonical/OG/Twitter/robots, no doubled brand) + removed the dead `<Head>` + unused import. v0.16.1, non-§3b (auth logic untouched), tsc + 317 tests green. This closes the last `<Head>` no-op among the sitemap'd static routes. Content chain still WP-blocked pending App-Password re-rotation.)
